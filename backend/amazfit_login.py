...
class HuamiAmazfit:
    ...
    def get_access_token(self) -> str:
        ...
        if self.method == "xiaomi":
            ...
            payload = urls.PAYLOADS["tokens_amazfit"] | {"password": self.password}
    ...
    def login(self) -> str:
        ...
        if "error_code" in info:
            raise RuntimeError(errors.ERRORS.get(info["error_code"], "unknown error"))
    ...
    def get_wearables(self):
        ...
        url = urls.URLS["devices"].format(user_id=urllib.parse.quote(self.user_id))
        headers = urls.PAYLOADS["devices"] | {"apptoken": self.app_token}
    ...
    def download_agps(self):
        ...
        hdr = urls.PAYLOADS["agps"] | {"apptoken": self.app_token}
        for pack, fname in agps_types:
            print(f"[+] downloading {fname}")
            res = requests.get(urls.URLS["agps"].format(pack_name=pack), headers=hdr)
            res.raise_for_status()
    ...
    def logout(self):
        data = urls.PAYLOADS["logout"] | {"login_token": self.login_token}
        res = requests.post(urls.URLS["logout"], data=data)
